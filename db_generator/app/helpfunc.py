...
'''
part one: read json file(1106 version), to output all chart as requested
'''
def read_jason(file_dir):
    '''
...
'''
part two: connect to db
please customize this part for different db 
'''
...
'''
part 3: handle query's 
'''
#read JSON file
argv1 = argv[1]
file_dir, host, database, user, password = argv1.split(',')

#split the lay_out into chart and markdown, using indicators in charts_list, comment_list
charts_list, comment_list, lay_out, lay_out_rows = read_jason(file_dir)
'''
for later use: tot_record,tot_chart_config, tot_x, tot_y: keys are 0,1,2,3...(int), type dict
                    -- only for chart 
                    -- e.g. tot_record = {0:[result in this section in dataframe]}
               tot_mark: keys are 0,1,2,3... (int), type dict
                    -- mark the markdowns 
               lay_out_list, remain unchanged from the previous part, type dict
'''
tot_record = {}
tot_chart_config = {}
tot_x = {}
    ...
    if _ in charts_list:
        ...
        if len(mySql_select_Query) != 0:
            ...
            for query in mySql_select_Query:
                ...
                # get query result
                db = Database(host, database, user, password, query)
                res = db.get_result()
